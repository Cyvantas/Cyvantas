...
/**
 * Centralized Motion presets for CYVANTAS.
 *
 * Timing follows CLAUDE.md: micro-interactions 150-250ms, larger
 * transitions 300-600ms. Reduced-motion is handled globally by wrapping
 * the app in <MotionConfig reducedMotion="user">, which collapses
 * transform-based motion to opacity-only for users who request it — so
 * these variants can safely declare y / scale transforms.
 */
...
export const pageTransition: Variants = {
  ...
  exit: {
    opacity: 0,
    y: -8,
    // Exit shorter than enter for responsiveness.
